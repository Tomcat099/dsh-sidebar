...
import { SCHEMA_MAX_DEPTH, SCHEMA_MAX_PROPERTIES, SCHEMA_MAX_TEXT, TOOL_CACHE_LIMIT, TOOL_SEARCH_LIMIT, } from './constants.js';
 ...
 */
export function readToolCache(profileDir) {
    const empty = { version: CACHE_VERSION, servers: {} };
    try {
        const parsed = JSON.parse(readFileSync(cachePath(profileDir), 'utf8'));
        if (typeof parsed !== 'object' || parsed === null)
            return empty;
        const raw = parsed;
        if (raw.version !== CACHE_VERSION)
            return empty;
        if (typeof raw.servers !== 'object' || raw.servers === null)
            return empty;
        const servers = {};
        for (const [key, value] of Object.entries(raw.servers)) {
            if (typeof value !== 'object' || value === null)
                continue;
            const item = value;
            if (typeof item.fetchedAt !== 'string' || typeof item.id !== 'string')
                continue;
 ...
 */
export function writeToolCache(profileDir, cache) {
 ...
 */
export function cacheTools(profileDir, server, tools, at = new Date().toISOString()) {
 ...
 */
export function pruneToolCache(profileDir, servers) {
 ...
 */
export function listCachedTools(profileDir, servers) {
    const cache = readToolCache(profileDir);
    ...
    for (const [key, entry] of Object.entries(cache.servers)) {
        ...
        const scope = server?.scope ?? entry.scope;
 ...
 */
export function searchCachedTools(tools, input) {
    const needle = input.query.trim().toLowerCase();
    const limit = Math.max(1, Math.min(input.limit ?? TOOL_SEARCH_LIMIT, 100));
    const scored = [];
    for (const tool of tools) {
        if (input.server !== undefined && input.server !== '' && tool.server !== input.server)
            continue;
        if (input.scope !== undefined && tool.scope !== input.scope)
            continue;
        if (needle === '') {
 ...
 */
export function findCachedTool(tools, input) {
 ...
 */
export async function refreshTools(profileDir, server, options = {}) {
 ...
 */
export function publicToolName(serverId, toolName) {
    if (!/^[A-Za-z0-9_-]{1,64}$/u.test(toolName))
        return undefined;
 ...
 */
export function trimSchema(schema, depth = 0) {
    if (schema === null || schema === undefined)
        return undefined;
    if (typeof schema === 'string')
        return schema.slice(0, SCHEMA_MAX_TEXT);
    ...
    for (const [key, value] of Object.entries(source)) {
        if (key === 'properties') {
            if (typeof value !== 'object' || value === null)
                continue;
            const trimmed = {};
            for (const [name, child] of Object.entries(value).slice(0, SCHEMA_MAX_PROPERTIES)) {
        ...
        if (key === 'description' || key === 'title') {
            if (typeof value === 'string')
                out[key] = value.slice(0, SCHEMA_MAX_TEXT);
            continue;
        }
 ...
 */
export function schemaSummary(tool) {
    const schema = tool.inputSchema;
    if (typeof schema !== 'object' || schema === null)
        return { properties: 0, required: [] };
    const raw = schema;
    const properties = typeof raw.properties === 'object' && raw.properties !== null
        ? Object.keys(raw.properties).length
        : 0;
