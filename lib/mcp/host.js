...
import { HealthStore, ensureHealth } from './health.js';
import { resolveDshHome, resolveProfileDir, scratchDir } from './paths.js';
import { registerRoutes } from './routes.js';
import { allServers, mountableServers, readContext } from './scope.js';
 ...
 */
async function loadInclude(profileDir) {
    ...
    if (loaded.Include === undefined)
        throw new Error('cordis-plugin-include 未导出 Include');
 ...
 */
function firstProjectRoot(ctx) {
    const listed = workspaceRegistry(ctx)?.list() ?? [];
    for (const item of listed) {
        const path = typeof item.path === 'string' ? item.path : '';
        if (path !== '' && !isJustChatPath(path))
            return path;
    }
    return undefined;
}
 ...
 */
function registeredToolNames(ctx) {
 ...
 */
async function mountOne(host, profileDir, entry, sequence) {
...
export function applyMcp(ctx) {
    ctx.inject(['webServer'], (host) => {
         ...
         * @returns the context.
         */
        const context = () => readContext(profileDir, activeProjectRoot);
        /**
        ...
        /**
         * Rebuild the mounted set when it differs from what is wanted.
         * @returns a promise that settles once the set matches the configuration.
         */
        const remount = async () => {
            ...
            if (signature === mountedSignature && live.size === wanted.length)
                return;
            await unmountAll();
            ...
            try {
                ...
                host.logger.warn('dsh-sidebar: MCP tool-cache prune failed: %s', error instanceof Error ? error.message : String(error));
            }
            for (const entry of wanted) {
                try {
                    ...
                    host.logger.warn('dsh-sidebar: MCP server "%s" failed to mount: %s', entry.id, error instanceof Error ? error.message : String(error));
                }
            }
         ...
         */
        const healthPass = async (options) => {
            ...
            const timer = setInterval(() => {
                void (async () => {
                    try {
                        ...
                        host.logger.warn('dsh-sidebar: MCP health sweep failed: %s', error instanceof Error ? error.message : String(error));
                    }
                })();
