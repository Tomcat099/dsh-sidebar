 ...
 */
export function documentExists(file) {
    try {
 ...
 */
export function writeDocument(file, doc, options = {}) {
 ...
 *
 * Indentation is relative to the row marker: the mapping keys line up two
 * spaces past the `-`, `config:`'s keys four, and their nested children six.
 * The absolute depths used to be hardcoded for the retired patch-layer
 * projection, which silently produced invalid YAML once this became a
 * standalone file — a regression the loader reported only as a mount failure.
 ...
 */
export function renderEntry(entry, indent = 0) {
    ...
    const lines = [
        ...
        `${key}name: '${MCP_CLIENT}'`,
        `${key}config:`,
        `${inner}serverName: ${entry.id}`,
        `${inner}transport: ${entry.transport}`,
    ];
    if (entry.transport === 'stdio') {
        lines.push(`${inner}command: ${quote(entry.command ?? '')}`);
        if (entry.args !== undefined && entry.args.length > 0) {
            lines.push(`${inner}args: [${entry.args.map(quote).join(', ')}]`);
        }
        if (entry.cwd !== undefined && entry.cwd !== '')
            lines.push(`${inner}cwd: ${quote(entry.cwd)}`);
        ...
        if (literal.length > 0 || referenced.length > 0) {
            ...
            for (const [name, value] of literal)
                lines.push(`${leaf}${name}: ${quote(value)}`);
            for (const [name, variable] of referenced) {
                lines.push(`${leaf}${name}: !!js process.env.${variable}`);
            }
        }
    }
    else {
        lines.push(`${inner}url: ${quote(entry.url ?? '')}`);
        const headers = headerPairs(entry);
        if (headers.length > 0) {
            lines.push(`${inner}headers:`);
            for (const [name, value] of headers)
                lines.push(`${leaf}${name}: ${value}`);
        }
    }
    return lines.join('\n');
}
 ...
 */
export function renderStandaloneEntry(entry) {
 ...
 */
export function stripManagedBlock(profileDir) {
