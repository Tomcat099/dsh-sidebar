...
function toEntry(name, value) {
    ...
    if (inferred === undefined)
        return { reason: `传输方式「${String(declared)}」无法识别` };
    const explicit = normalizeEntry({ ...raw, transport: inferred, id });
    if (explicit === undefined)
        return { reason: '缺少 command 或 url，无法判断传输方式' };
    const entry = {
        ...explicit,
        id,
    ...
    if (env.values !== undefined)
        entry.env = env.values;
    else
        delete entry.env;
    if (env.refs !== undefined)
        entry.envEnv = { ...(entry.envEnv ?? {}), ...env.refs };
    const headers = splitReferences(asStringMap(raw.headers));
    if (headers.values !== undefined)
        entry.headers = headers.values;
    else
        delete entry.headers;
    if (headers.refs !== undefined)
        entry.headerEnv = { ...(entry.headerEnv ?? {}), ...headers.refs };
...
function extractServers(parsed) {
    if (Array.isArray(parsed))
        return { list: parsed.map(item => [undefined, item]) };
    if (typeof parsed !== 'object' || parsed === null)
        return { reason: '顶层不是对象或数组' };
    const raw = parsed;
    for (const key of ['mcpServers', 'servers', 'connections']) {
        ...
        if (Array.isArray(value))
            return { list: value.map(item => [undefined, item]) };
        if (typeof value === 'object' && value !== null)
            return { list: Object.entries(value) };
        return { reason: `「${key}」不是对象或数组` };
    }
    if (raw.url !== undefined || raw.command !== undefined)
        return { list: [[undefined, parsed]] };
...
export function parseImportJson(text) {
    ...
    if (trimmed === '')
        return { ok: false, message: '内容是空的', entries: [], issues: [] };
    let parsed;
    try {
    ...
    if ('reason' in extracted)
        return { ok: false, message: extracted.reason, entries: [], issues: [] };
    if (extracted.list.length === 0)
        return { ok: false, message: '里面没有任何服务器', entries: [], issues: [] };
    const entries = [];
    const issues = [];
    const skipped = [];
    const seen = new Set();
 ...
 */
export function importIntoDocument(existing, entries, mode = 'merge') {
 ...
 */
export function exportRedacted(doc) {
    let redacted = 0;
    const mcpServers = {};
    for (const entry of doc.servers) {
        ...
        if (entry.transport === 'stdio') {
            ...
            if (entry.envEnv !== undefined && Object.keys(entry.envEnv).length > 0)
                out.envEnv = { ...entry.envEnv };
        }
        else {
            ...
            if (entry.headerEnv !== undefined && Object.keys(entry.headerEnv).length > 0)
                out.headerEnv = { ...entry.headerEnv };
        }
        if (entry.toolsets !== undefined && entry.toolsets !== '')
            out.toolsets = entry.toolsets;
        if (entry.readonly === true)
            out.readonly = true;
        if (entry.insecurePrivateNetwork === true)
            out.insecurePrivateNetwork = true;
