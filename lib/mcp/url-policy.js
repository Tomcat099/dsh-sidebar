...
function classifyIpv6(text) {
    ...
    if (text === '::1' || text === '0:0:0:0:0:0:0:1')
        return 'loopback';
    if (text === '::')
        return 'link-local';
    ...
    if (Number.isNaN(value))
        return undefined;
    if ((value & 0xffc0) === 0xfe80 || (value & 0xffc0) === 0xfec0)
        return 'link-local';
 ...
 */
export function checkUrl(raw, options = {}) {
    const text = raw.trim();
    if (text === '') {
 ...
 */
export function isRiskConfirmable(code) {
