 ...
 */
function renderLines(hits, budget, now) {
    const lines = [];
    let used = 0;
 ...
 */
export function renderRecallBlock(hits, now = Date.now()) {
 ...
 * @returns Markdown, or `''` when there is nothing to inject.
 */
export function renderStandingBlock(hits, now = Date.now()) {
 ...
 */
export function standingHits(hits) {
...
export class RecallCache {
    ...
    get(sessionId) {
        ...
        if (sessionId !== undefined && sessionId !== '') {
            ...
            if (session === '') {
                for (const value of this.blocks.values())
                    session = value;
            }
        }
        if (this.standing === '')
