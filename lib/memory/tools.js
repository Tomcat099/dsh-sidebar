...
function requireProvider(host) {
    ...
    if (provider === undefined)
        throw new Error(host.complaint() ?? '记忆功能不可用');
    return provider;
}
 ...
 */
export function memoryTools(host) {
    return [
        {
            name: 'memory_search',
            ...
            async execute(args) {
                ...
                const requested = Number(args?.topK);
        ...
        },
        {
            ...
            output: {
                ...
                render: (_args, value) => [{
                        type: 'text',
                        text: value.created ? '已记住这条信息。' : '这条信息已在记忆中，已更新。',
                    }],
            },
            timeoutMs: 20_000,
            async execute(args) {
