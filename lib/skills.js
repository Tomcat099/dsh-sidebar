 ...
 */
export function skillRoots(projectRoot, customSkillDirs, bundledSkillDir) {
    const roots = [
        { path: join(projectRoot, '.dsh', 'skills'), rank: 100, scope: 'project' },
        { path: join(projectRoot, '.agents', 'skills'), rank: 200, scope: 'project' },
        ...customSkillDirs.map((path) => ({ path: resolve(path), rank: 300, scope: 'custom' })),
 ...
 */
export async function listInstallCandidates(projectRoot) {
    ...
    for (const entry of entries) {
        ...
        if (parsed === undefined || parsed.name !== entry.name)
            continue;
 ...
 */
export async function installSkill(sourceDir, scope, projectRoot) {
    const source = resolve(sourceDir);
    const info = await follow(source);
    if (info === undefined || !info.isDirectory())
        throw new SkillError(400, '来源目录不存在');
    if (!SKILL_NAME.test(basenameOf(source)))
        throw new SkillError(400, '技能目录名必须是 kebab-case');
    const raw = await readText(join(source, 'SKILL.md'));
    if (raw === undefined)
        throw new SkillError(400, '来源目录缺少 SKILL.md');
    const parsed = readSkillMeta(raw, join(source, 'SKILL.md'));
    if (parsed === undefined)
        throw new SkillError(400, 'SKILL.md 无法解析');
    if (parsed.name !== basenameOf(source))
        throw new SkillError(400, '目录名与 frontmatter name 不一致');
    const targetRoot = scope === 'project' ? join(projectRoot, '.dsh', 'skills') : globalSkillRoot();
    const target = join(targetRoot, parsed.name);
    if (resolve(source) === resolve(target))
        throw new SkillError(409, 'already-installed');
 ...
 * @returns the installed directory.
 */
export async function installFromUpload(parts, scope, projectRoot) {
    if (parts.length === 0)
        throw new SkillError(400, '没有收到文件');
    const total = parts.reduce((sum, part) => sum + part.bytes.byteLength, 0);
    if (total > MAX_UPLOAD_BYTES)
        throw new SkillError(400, '文件过大');
    const scratch = await mkdtemp(join(tmpdir(), 'dsh-skill-upload-'));
    try {
        const only = parts.length === 1 ? parts[0] : undefined;
        if (only !== undefined && isArchiveName(only.name))
            await extractArchive(only.bytes, only.name, scratch);
        else {
            for (const part of parts)
                await writeSafe(scratch, part.name, part.bytes);
        }
 ...
 */
export async function toggleSkill(sourceDir, enabled, projectRoot) {
    const file = await resolveSkillFile(sourceDir);
    if (file === undefined)
        throw new SkillError(404, '找不到 SKILL.md');
    if (!isWritableSkill(file, projectRoot))
        throw new SkillError(403, '这一层只读，不能改开关');
    const raw = await readFile(file, 'utf8');
    const next = rewriteInvocation(raw, enabled);
    if (next === undefined)
        throw new SkillError(400, 'SKILL.md 没有合法的 frontmatter');
 ...
 */
export function rewriteInvocation(raw, enabled) {
...
async function scanRoot(root) {
    ...
    const skills = [];
    for (const entry of entries) {
        if (root.skipSystem === true && entry.name === '.system')
            continue;
...
async function readSkillView(file, sourceDir, root) {
    const raw = await readText(file);
    if (raw === undefined)
        return undefined;
    const parsed = readSkillMeta(raw, file);
...
function readSkillMeta(raw, file) {
    const located = locateFrontmatter(raw);
    if (located === undefined) {
...
function coerceScalar(raw) {
    ...
    if (unquoted === raw.trim()) {
        ...
        if (folded === 'true' || folded === 'yes' || folded === 'on')
            return true;
        if (folded === 'false' || folded === 'no' || folded === 'off')
            return false;
    }
    return unquoted;
}
...
function locateFrontmatter(raw) {
    const newline = raw.includes('\r\n') ? '\r\n' : '\n';
    const firstBreak = raw.indexOf('\n');
    if (firstBreak < 0)
        return undefined;
    if (raw.slice(0, firstBreak).replace(/\r$/, '') !== '---')
        return undefined;
...
async function resolveSkillFile(sourceDir) {
...
function isWritableSkill(file, projectRoot) {
...
export function assertAbsolute(path) {
    if (!isAbsolute(path))
        throw new SkillError(400, '路径必须是绝对路径');
    return resolve(path);
}
...
async function writeSafe(dest, name, bytes) {
    const relative = safeRelative(name);
    if (relative === undefined)
        throw new SkillError(400, '文件路径不合法');
    if (relative.split('/').includes('__MACOSX') || relative.endsWith('.DS_Store'))
        return;
    const target = join(dest, relative);
    await mkdir(dirname(target), { recursive: true });
    await writeFile(target, bytes);
}
async function findUploadedSkill(root) {
    const files = [];
    await collectSkillFiles(root, files);
    if (files.length === 0)
        throw new SkillError(400, '找不到 SKILL.md');
    ...
    if (dirs.length !== 1)
        throw new SkillError(400, '一次只能安装一个技能');
    const dir = dirs[0];
    if (dir === undefined)
        throw new SkillError(400, '找不到 SKILL.md');
    if (resolve(dir) === resolve(root))
        return relocateRootSkill(root);
    return dir;
}
...
async function relocateRootSkill(root) {
    const file = join(root, 'SKILL.md');
    const raw = await readText(file);
    if (raw === undefined)
        throw new SkillError(400, '找不到 SKILL.md');
    const parsed = readSkillMeta(raw, file);
    if (parsed === undefined || !SKILL_NAME.test(parsed.name))
        throw new SkillError(400, 'SKILL.md 无法解析');
...
async function extractArchive(bytes, name, dest) {
...
async function extractZip(bytes, dest) {
    const view = Buffer.from(bytes);
    const eocd = findEocd(view);
    if (eocd < 0)
        throw new SkillError(400, '压缩包无法解析');
    ...
    for (let index = 0; index < count; index += 1) {
        ...
        if (name.endsWith('/') || name.endsWith('\\'))
            continue;
        if (compSize > MAX_UPLOAD_BYTES || rawSize > MAX_UPLOAD_BYTES)
            throw new SkillError(400, '压缩包过大');
        total += rawSize;
        if (total > MAX_UPLOAD_BYTES)
            throw new SkillError(400, '压缩包过大');
        ...
        if (raw === undefined)
            throw new SkillError(400, '压缩包使用了不支持的压缩方式');
...
async function extractTar(view, dest) {
    let offset = 0;
    let total = 0;
    let longName;
    while (offset + 512 <= view.length) {
        if (view.subarray(offset, offset + 512).every((byte) => byte === 0))
            break;
...
function tarString(view, start, length) {
