 ...
 */
function filesystemConfig(ctx: Context): { customSkillDirs: string[], bundledSkillDir: string | undefined } {
  const custom = new Set<string>()
  let bundled = process.env.DSH_BUNDLED_SKILL_DIR
  let includeDefaults = true
  let explicitBundled = false
  for (const runtime of ctx.registry.values()) {
    if (runtime.name !== 'skill-filesystem' && runtime.name !== '@deepseek-ai/dsh-skill-filesystem') continue
    for (const fiber of runtime.fibers) {
      if (typeof fiber.config !== 'object' || fiber.config === null) continue
      const config = fiber.config as {
        customSkillDirs?: unknown
        bundledSkillDir?: unknown
        includeDefaultRoots?: unknown
      }
      if (Array.isArray(config.customSkillDirs)) {
        for (const dir of config.customSkillDirs) {
          if (typeof dir === 'string' && dir !== '') custom.add(resolve(dir))
        }
      }
      ...
      if (config.includeDefaultRoots === false) includeDefaults = false
    }
  }
 ...
 */
function startCwd(ctx: Context, raw: unknown): string {
  if (typeof raw === 'string' && raw !== '' && isAbsolute(raw)) return resolve(raw)
...
function registerSkills(ctx: Context): void {
  ...
  ctx.connection.fetch.register({
    ...
    async fetch(request) {
      try {
        request.signal.throwIfAborted()
        const body = await request.json() as { sourceDir?: unknown, scope?: unknown, cwd?: unknown }
  ...
  ctx.connection.fetch.register({
    ...
    async fetch(request) {
      try {
        request.signal.throwIfAborted()
        if (typeof request.formData !== 'function') return Response.json({ error: '上传内容无法解析' }, { status: 400 })
        ...
        const files: Awaited<ReturnType<typeof uploadPart>>[] = []
        for (const item of form.getAll('files')) files.push(await uploadPart(item))
  ...
  ctx.connection.fetch.register({
    ...
    async fetch(request) {
      try {
        request.signal.throwIfAborted()
        const body = await request.json() as { sourceDir?: unknown, enabled?: unknown, cwd?: unknown }
        if (typeof body.sourceDir !== 'string' || !isAbsolute(body.sourceDir)) {
          return Response.json({ error: 'sourceDir 必须是绝对路径' }, { status: 400 })
        }
...
function skillFailure(error: unknown): Response {
  if (error instanceof SkillError) return Response.json({ error: error.message }, { status: error.status })
 ...
 */
async function uploadPart(value: unknown): Promise<{ name: string, bytes: Uint8Array } | undefined> {
  if (value === null || typeof value !== 'object' || !('arrayBuffer' in value)) return undefined
 ...
 * @param ctx - host context.
 */
function registerRecent(ctx: Context): void {
  ctx.connection.fetch.register({
    path: PREPARE_ROUTE,
    methods: ['POST'],
    requestBody: 'buffered',
