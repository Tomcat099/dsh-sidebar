...
import type { McpServerEntry, ScopedServer } from './schema.js'
import { displayName, endpointLabel } from './schema.js'
import type { ProbeKind, ProbeResult } from './probe.js'

/** One of the eight states a connection can be reported in. */
export type ConnectionState =
  /** Nothing configured for this scope. */
  | 'disconnected'
  /** The Host has not told us anything yet; do not claim health. */
  | 'unknown'
  /** Configured but switched off; it is not mounted. */
  | 'disabled'
  /** Handshake and tool listing both succeeded. */
  | 'healthy'
  /** Some servers work and some do not. */
  | 'degraded'
  /** Credentials were refused; automatic retries are paused. */
  | 'reauth'
  /** A transient failure; an automatic retry is scheduled. */
  | 'recovering'
  /** Failed for a reason that retrying will not fix by itself. */
  | 'unavailable'

/** Where in the connection pipeline a failure happened. */
...
export const CONNECTION_STATE_LABELS: Readonly<Record<ConnectionState, string>> = Object.freeze({
  disconnected: '未配置',
  unknown: '状态未知',
  disabled: '已停用',
  healthy: '已连接',
...
export const STAGE_LABELS: Readonly<Record<FailureStage, string>> = Object.freeze({
  configuration: '配置',
  authentication: '鉴权',
  transport: '网络与传输',
 ...
 */
export const FAILURE_GUIDANCE: Readonly<Record<string, readonly [FailureStage, string]>> = Object.freeze({
  // Configuration
...
const FALLBACK_GUIDANCE: readonly [FailureStage, string] = ['transport', '检查网络与 MCP URL 是否正确']

/** What the UI shows under one row. */
export interface Diagnostic {
  /** State implied by the failure, or `healthy` on success. */
  state: ConnectionState
  /** Pipeline stage the failure belongs to. */
  stage: FailureStage
  /** Chinese label for {@link stage}. */
  stageLabel: string
  ...
  checkedAt: string
  /** Last time a handshake on this entry succeeded; absent when never. */
  lastSuccessfulAt?: string
}

/** One entry's live state, as the settings page renders it. */
export interface ServerHealth extends Diagnostic {
  /** Server id. */
  id: string
  /** Display name. */
  name: string
  /** Chinese label for {@link Diagnostic.state}, so the row does not re-derive it. */
  label: string
  /** Scope the entry was read from. */
  scope: ScopedServer['scope']
  /** Transport in use. */
  transport: McpServerEntry['transport']
  /** Whether the entry is mounted at all. */
  enabled: boolean
  /** One-line endpoint summary. */
  endpoint: string
  /** True only when the handshake and tool listing both succeeded. */
  ok: boolean
  /** Tools discovered on the last successful listing. */
  toolCount?: number
  /** Epoch milliseconds before which no retry is attempted. */
  retryAt?: number
  /** True when an authentication failure stopped automatic retries. */
  paused?: boolean
  /** Protocol revision the server acknowledged. */
  protocolVersion?: string
  /** Server name as reported during `initialize`. */
  serverName?: string
  /** Server version as reported during `initialize`. */
  serverVersion?: string
}

/** Aggregated view of every entry in the current scope set. */
export interface HealthSummary {
  /** Overall state across the entries that were summarized. */
  connectionState: ConnectionState
  /** Chinese label for {@link connectionState}. */
  label: string
  /** Entries configured. */
  configured: number
  /** Entries switched on. */
  enabled: number
  /** Entries whose handshake succeeded. */
  available: number
  /** Entries handed to the Host whose outcome is unobserved. */
  pending: number
  /** Entries whose check failed. */
  failed: number
  /** Failures caused by credentials, which pause automatic retries. */
  authFailures: number
  /** When the summary was produced. */
  checkedAt: string
  /** Most recent success across all entries, when any. */
  lastSuccessfulAt?: string
  /** The single diagnostic that best explains the overall state. */
 ...
 */
export function stageOf(kind: ProbeKind): FailureStage {
  if (kind === 'connected') return 'ready'
  if (kind === 'managed') return 'host-observation'
  if (kind === 'authorization-required' || kind === 'auth') return 'authentication'
  if (kind === 'protocol') return 'mcp-initialize'
  if (kind === 'process-not-found' || kind === 'process-exit' || kind === 'startup') return 'host-startup'
  if (kind.startsWith('host-')) return 'host-observation'
  if (kind === 'tool-list') return 'tool-discovery'
  return 'transport'
}
 ...
 */
export function stateOfFailure(kind: ProbeKind): ConnectionState {
  if (kind === 'auth' || kind === 'authorization-required') return 'reauth'
  if (kind === 'rate-limit') return 'recovering'
  if (kind === 'managed') return 'unknown'
  return 'unavailable'
}
 ...
 */
export function diagnosticFor(result: ProbeResult, lastSuccessfulAt?: string): Diagnostic {
  const kind: ProbeKind = result.kind
 ...
 */
export function unobservedDiagnostic(checkedAt: string, lastSuccessfulAt?: string): Diagnostic {
  return {
    state: 'unknown',
 ...
 */
export function disabledDiagnostic(checkedAt: string): Diagnostic {
  return {
    state: 'disabled',
    stage: 'configuration',
...
export function healthRow(
  server: ScopedServer,
  result: ProbeResult,
  extras: { lastSuccessfulAt?: string, retryAt?: number, paused?: boolean } = {},
): ServerHealth {
  const diagnostic = diagnosticFor(result, extras.lastSuccessfulAt)
  const row: ServerHealth = {
    ...diagnostic,
    id: server.id,
  ...
  if (result.tools !== undefined) row.toolCount = result.tools.length
  if (extras.retryAt !== undefined) row.retryAt = extras.retryAt
  if (extras.paused === true) row.paused = true
  if (result.protocolVersion !== undefined) row.protocolVersion = result.protocolVersion
  if (result.serverInfo?.name !== undefined) row.serverName = result.serverInfo.name
  if (result.serverInfo?.version !== undefined) row.serverVersion = result.serverInfo.version
  return row
}
 ...
 * @returns the summary.
 */
export function summarize(rows: readonly ServerHealth[], checkedAt = new Date().toISOString()): HealthSummary {
  ...
  if (configured === 0) connectionState = 'disconnected'
  else if (enabled === 0) connectionState = 'disabled'
  else if (failed === 0 && pending === 0 && available > 0) connectionState = 'healthy'
  else if (available > 0 && (failed > 0 || pending > 0)) connectionState = 'degraded'
  else if (pending > 0 && failed === 0) connectionState = 'unknown'
  else if (authFailures > 0) connectionState = 'reauth'
  else if (recovering > 0) connectionState = 'recovering'
  ...
  if (lastSuccessfulAt !== undefined) summary.lastSuccessfulAt = lastSuccessfulAt
  return summary
}
 ...
 */
function overallDiagnostic(
  rows: readonly ServerHealth[],
  connectionState: ConnectionState,
  checkedAt: string,
  lastSuccessfulAt?: string,
): Diagnostic {
  ...
  if (connectionState === 'disabled') return disabledDiagnostic(checkedAt)
  if (connectionState === 'unknown') return unobservedDiagnostic(checkedAt, lastSuccessfulAt)
