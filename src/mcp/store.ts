...
export function renderEntry(entry: McpServerEntry, indent = 0): string {
  ...
  if (entry.transport === 'stdio') {
    ...
    if (entry.cwd !== undefined && entry.cwd !== '') lines.push(`${inner}cwd: ${quote(entry.cwd)}`)
    ...
    if (literal.length > 0 || referenced.length > 0) {
      ...
      for (const [name, value] of literal) lines.push(`${leaf}${name}: ${quote(value)}`)
    ...
    if (headers.length > 0) {
      ...
      for (const [name, value] of headers) lines.push(`${leaf}${name}: ${value}`)
    }
  }
  return lines.join('\n')
}
