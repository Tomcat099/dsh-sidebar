 ...
 */
export const MEMORY_CONFIG_KEYS: ReadonlyArray<keyof MemoryConfig> = [
  'enabled',
  'mode',
  'localDir',
  'localDb',
...
export interface MemoryConfig {
  ...
  embeddingDim: number
  externalKind: 'http' | 'mcp'
  externalBaseURL: string
  externalApiKey: string
  externalNamespace: string
  externalSearchTool: string
  externalWriteTool: string
  autoExtract: boolean
  autoRecall: boolean
  recallTopK: number
  extractModel: string
  extractMode: 'auto' | 'explicit'
  extractKeywords: string
  extractSource: 'user' | 'both'
  extractKinds: string
  extractFocus: string
 ...
 */
export function resolveMemoryConfig(section: unknown): MemoryConfig {
  const raw = (section ?? {}) as Partial<Record<keyof MemoryConfig, unknown>>
  const pick = <K extends keyof MemoryConfig>(key: K): MemoryConfig[K] => {
    const value = raw[key]
    return (value === undefined || value === null ? DEFAULT_MEMORY_CONFIG[key] : value) as MemoryConfig[K]
  }
 ...
 */
export function resolveMemoryDir(config: MemoryConfig): string {
 ...
 */
export function resolveMemoryDb(config: MemoryConfig): string {
 ...
 */
export function memoryConfigComplaint(config: MemoryConfig): string | undefined {
  if (!config.enabled) return '记忆功能未启用'
  if (config.mode === 'external') {
    if (config.externalBaseURL.trim() === '') return '外部存储未填写服务地址'
    return undefined
  }
  if (config.localDb.trim() === '') return '本地存储未填写数据库文件名'
  return undefined
}
