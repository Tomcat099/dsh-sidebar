...
/** Message roles kept in a transcript. */
...
export const ALLOWED_KINDS = ['preference', 'fact', 'decision', 'entity', 'convention'] as const
...
/** What is worth remembering, as configured by the user. */
export interface ExtractionRules {
  /** `auto` extracts every settled turn; `explicit` only when asked to remember. */
  mode: 'auto' | 'explicit'
  /** Which speakers may contribute facts. */
  source: 'user' | 'both'
  /** Allowed kinds; empty means every kind. */
  kinds: string[]
  /** Extra "what to remember" requirements, passed to the model. */
  focus: string
  /** "What never to remember" requirements, passed to the model. */
  exclude: string
  /** Minimum confidence accepted. */
  minConfidence: number
  /** Most facts taken from one turn. */
  maxFacts: number
  /** Trigger words for the explicit mode. */
  keywords: string[]
}
...
/** One model call, injected so extraction stays runtime-agnostic. */
export type CompletionRunner = (input: {
  system: string
  user: string
  signal: AbortSignal
}) => Promise<string>

/** Inputs of one extraction pass. */
export interface ExtractionRequest {
  transcript: TranscriptLine[]
  /** Contents already stored, used to suppress duplicates. */
  existing: string[]
  /** Remembrance policy; defaults apply when absent. */
 ...
 */
export function hasExplicitRequest(texts: string[], keywords: string[]): boolean {
 ...
 */
export function renderTranscript(transcript: TranscriptLine[], budget = TRANSCRIPT_BUDGET): string {
  const lines: string[] = []
  let used = 0
  for (let index = transcript.length - 1; index >= 0; index -= 1) {
    ...
    if (used + rendered.length > budget && lines.length > 0) break
 ...
 */
export function extractionPrompts(request: ExtractionRequest): { system: string, user: string } {
 ...
 */
function firstJsonArray(raw: string): unknown[] {
 ...
 */
export function parseExtraction(raw: string, rules: ExtractionRules = DEFAULT_EXTRACTION_RULES): MemoryDraft[] {
  ...
  for (const item of firstJsonArray(raw)) {
    ...
    const kind = (ALLOWED_KINDS as readonly string[]).includes(rawKind) ? rawKind : 'fact'
    if (!allowed.has(kind)) continue
    const content = typeof record.content === 'string' ? record.content : ''
    if (!isStorable(kind, content)) continue
    ...
    if (value + 1e-9 < rules.minConfidence) continue
...
export async function extractMemories(
  ...
  if (request.transcript.length === 0) return []
  const rules = request.rules ?? DEFAULT_EXTRACTION_RULES
...
export function collectTurnMessages(
  ...
  sinceSeq: number,
  options: { maxLines?: number, source?: 'user' | 'both' } = {},
): { lines: TranscriptLine[], lastSeq: number } {
  ...
  for (const event of events) {
    ...
    const role: TranscriptRole | undefined = event.type === 'user/message'
      ? 'user'
    ...
    if (role === 'assistant' && options.source === 'user') continue
 ...
 */
function messageText(event: { data?: unknown }): string {
