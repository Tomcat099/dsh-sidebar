...
/** Endpoint settings shared by both remote protocols. */
...
async function requestJson(
  url: string,
  init: { method: string, body?: unknown, apiKey?: string, accept?: string },
  ...
  try {
    ...
    if (init.apiKey !== undefined && init.apiKey.trim() !== '') headers.authorization = `Bearer ${init.apiKey.trim()}`
...
function readTime(value: unknown, fallback: number): number {
  if (typeof value === 'number' && Number.isFinite(value)) return value < 1e12 ? Math.trunc(value * 1000) : Math.trunc(value)
  if (typeof value === 'string') {
    const parsed = Date.parse(value)
    if (Number.isFinite(parsed)) return parsed
    const numeric = Number(value)
    if (Number.isFinite(numeric)) return numeric < 1e12 ? Math.trunc(numeric * 1000) : Math.trunc(numeric)
  }
  return fallback
}
 ...
 */
function toHit(row: unknown): MemoryHit | undefined {
  ...
  return {
    ...
    content,
    tags,
    ...(readString(metadata.source_session) === undefined ? {} : { sourceSession: readString(metadata.source_session) as string }),
    ...(readString(metadata.source_workspace) === undefined ? {} : { sourceWorkspace: readString(metadata.source_workspace) as string }),
 ...
 */
function readRows(body: unknown): unknown[] {
  if (Array.isArray(body)) return body
  const record = body as { results?: unknown, memories?: unknown, data?: unknown } | undefined
 ...
 * @returns the id.
 */
function readCreatedId(body: unknown, fallback: string): string {
  const record = body as { id?: unknown, memory_id?: unknown, results?: unknown } | undefined
...
export class HttpMemoryProvider implements MemoryProvider {
  ...
  async health(): Promise<MemoryHealth> {
    try {
      const { response } = await requestJson(
        `${this.base}/memories?user_id=${encodeURIComponent(this.settings.namespace)}&limit=1`,
        { method: 'GET', apiKey: this.settings.apiKey },
        8000,
      )
      return response.ok
  ...
  async write(draft: MemoryDraft): Promise<{ id: string, created: boolean }> {
    ...
    if (!response.ok) throw new Error(`外部记忆写入失败：HTTP ${String(response.status)}`)
  ...
  async search(query: MemoryQuery): Promise<MemoryHit[]> {
    ...
    if (!response.ok) throw new Error(`外部记忆检索失败：HTTP ${String(response.status)}`)
  ...
  async list(options: MemoryListOptions): Promise<MemoryHit[]> {
    ...
    if (!response.ok) throw new Error(`外部记忆列表失败：HTTP ${String(response.status)}`)
    const text = (options.text ?? '').trim().toLowerCase()
    return readRows(body).flatMap((row) => {
      const hit = toHit(row)
      if (hit === undefined) return []
      if (text !== '' && !hit.content.toLowerCase().includes(text) && !hit.tags.some((tag) => tag.toLowerCase().includes(text))) return []
      return [hit]
    })
  }
  ...
  async update(id: string, patch: Partial<MemoryDraft>): Promise<void> {
    ...
    if (!response.ok) throw new Error(`外部记忆更新失败：HTTP ${String(response.status)}`)
  }
  ...
  async remove(id: string): Promise<void> {
    ...
    if (!response.ok) throw new Error(`外部记忆删除失败：HTTP ${String(response.status)}`)
  }
}

/** MCP tools over streamable HTTP. */
export class McpMemoryProvider implements MemoryProvider {
  readonly kind = 'mcp' as const
  private readonly settings: RemoteSettings
  private sequence = 0
  private tools: { search: string, write: string } | undefined
   ...
   */
  private async resolveTools(): Promise<{ search: string, write: string }> {
    ...
    const names = Array.isArray(tools)
      ...
      : []
    if (names.length === 0) throw new Error('MCP 服务没有提供任何工具，无法自动选择记忆工具')
    const pick = (patterns: RegExp[], exclude: string | undefined): string | undefined => {
    ...
    const search = configuredSearch !== ''
      ? configuredSearch
      : pick([/search|query|find|recall|retrieve/i], undefined) ?? names[0] as string
  ...
  private async call(name: string, args: Record<string, unknown>): Promise<unknown> {
    ...
    if (!response.ok) throw new Error(`MCP 调用失败：HTTP ${String(response.status)}`)
    const record = body as { error?: { message?: unknown }, result?: unknown } | undefined
    if (record?.error !== undefined) throw new Error(`MCP 返回错误：${String(record.error.message ?? '未知错误')}`)
