...
import type { RankedCandidate, RankedList } from './types.js'
 ...
 */
export function isStorable(kind: string, content: string): boolean {
 ...
 */
export function ftsExpression(text: string): string | undefined {
 ...
 */
export function likeTerm(text: string): string | undefined {
  const terms = ftsTerms(text)
  if (terms.length === 0) return undefined
 ...
 */
export function compactEntries<T extends { kind: string, content: string }>(entries: T[], maxLength = 400): T[] {
  const accepted: T[] = []
  for (const entry of entries) {
    const content = normalizeContent(entry.content)
    if (!isStorable(entry.kind, content)) continue
    if (content.length > maxLength) continue
...
export function fuse(
  lists: RankedList[],
  options: { k?: number, topK: number } = { topK: 5 },
): Array<RankedCandidate & { sources: string[] }> {
  ...
  for (const list of lists) {
    list.candidates.forEach((candidate, index) => {
      ...
      const found = sources.get(candidate.id) ?? new Set<string>()
      found.add(list.name)
 ...
 */
export function encodeVector(vector: ArrayLike<number>): Uint8Array {
  const floats = new Float32Array(vector.length)
  for (let index = 0; index < vector.length; index += 1) floats[index] = vector[index] ?? 0
...
export function decodeVector(value: unknown): Float32Array {
  if (!(value instanceof Uint8Array)) return new Float32Array(0)
  if (value.byteLength === 0 || value.byteLength % 4 !== 0) return new Float32Array(0)
