...
/** One prepared statement, as narrow as this plugin needs. */
...
export function migrate(db: SqlDatabase): number {
  ...
  if (current < 2) rehashContents(db)
  if (current < SCHEMA_VERSION) writeVersion(db, SCHEMA_VERSION)
 ...
 * @param db - open database.
 */
function rehashContents(db: SqlDatabase): void {
  const rows = db.prepare('SELECT id, content, tags, content_hash FROM memories ORDER BY created_at ASC')
    .all() as Array<{ id: string, content: string, tags: string, content_hash: string }>
...
function readVersion(db: SqlDatabase): number {
  const row = db.prepare('SELECT value FROM memory_meta WHERE key = ?').get('schema_version') as { value?: unknown } | undefined
 ...
 */
export function readCounter(db: SqlDatabase, key: string): number {
  const row = db.prepare('SELECT value FROM memory_meta WHERE key = ?').get(key) as { value?: unknown } | undefined
  const parsed = Number(row?.value)
 ...
 */
export function writeCounter(db: SqlDatabase, key: string, value: number): void {
  db.prepare('INSERT INTO memory_meta(key, value) VALUES(?, ?) ON CONFLICT(key) DO UPDATE SET value = excluded.value')
