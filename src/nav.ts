...
/** One workspace row the sidebar can group or hide. */
export interface WorkspaceRef {
  /** Host workspace id. */
  workspaceId: string
  /** Absolute directory registered with the host. */
  path: string
  /** Display title. */
  title: string
  /** Sessions accounted to this workspace, in host order. */
  sessionIds: readonly string[]
}

/** One session row in the recent list. */
export interface SessionRef {
  /** Host session id. */
  id: string
  /** Display title. A blank session is the new chat before the first message. */
  title: string
  /** Last activity, epoch milliseconds. */
  updatedAt: number
  /** True while the session has no turn yet. */
  blank?: boolean
}
 ...
 */
export function isJustChatPath(path: string): boolean {
 ...
 */
export function projectWorkspaces(items: readonly WorkspaceRef[]): WorkspaceRef[] {
 ...
 */
export function justChatWorkspaces(items: readonly WorkspaceRef[]): WorkspaceRef[] {
  return items.filter(item => isJustChatPath(item.path))
}
...
export function currentProjectId(
  items: readonly WorkspaceRef[],
  currentSessionId: string | undefined,
): string | undefined {
  if (currentSessionId === undefined || currentSessionId === '') return undefined
  return items.find(item =>
    !isJustChatPath(item.path) && item.sessionIds.includes(currentSessionId),
  )?.workspaceId
}
...
export function recentSessions(
  ...
  sessions: Readonly<Record<string, SessionRef | undefined>>,
 ...
 */
export function justChatTitle(now: Date = new Date()): string {
